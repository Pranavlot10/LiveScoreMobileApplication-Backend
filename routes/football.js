...
router.get("/todays-matches", async (req, res) => {
  ...
  try {
    const cachedMatchesKey = `${date}/${month}/${year}MatchesKey`;
    ...
    const FootApiKeyData = await getAvailableApiKey("footApi");
    console.log(`Using API Key: ${FootApiKeyData.api_key_id}`);

    let headers = {
      "x-rapidapi-key": FootApiKeyData.value,
      "x-rapidapi-host": "footapi7.p.rapidapi.com",
    };

    const matchesResponse = await axios.get(
      `${FOOTAPI_URL}/api/matches/top/${date}/${month}/${year}`,
      { headers }
    );
    ...
    for (const match of matchesData) {
      for (const teamKey of ["homeTeam", "awayTeam"]) {
        ...
        if (team && team.id) {
          const teamLogo = await fetchTeamLogo(team.id, team.name);
      ...
      if (
        ...
        const leagueLogo = await fetchLeagueLogo(id, name, country);
    ...
    await updateApiKeyUsage(FootApiKeyData.id, 1);
  } catch (error) {
    console.error(
...
router.get("/details/:matchId", async (req, res) => {
  ...
  try {
    ...
    // Fetch data from API
    const [incidentsResponse, lineupsResponse, matchInfoResponse] =
      await Promise.all([
        axios.get(`${FOOTAPI_URL}/api/match/${matchId}/incidents`, { headers }),
        axios.get(`${FOOTAPI_URL}/api/match/${matchId}/lineups`, { headers }),
        axios.get(`${FOOTAPI_URL}/api/match/${matchId}`, { headers }),
      ]);
    ...
    const matchesData = {
      ...
      matchInfoData: matchInfoResponse.data || {},
    };
    ...
    await updateApiKeyUsage(FootApiKeyData.id, 3);
  } catch (error) {
    console.error(
...
router.get("/commentary/:matchId", async (req, res) => {
  ...
  try {
    const cachedMatchKey = `${matchId}commentary`;
    ...
    // Fetch data from API
    const [commentaryResponse] = await Promise.all([
      axios.get(`${FOOTAPI_URL}/api/match/${matchId}/commentary`, { headers }),
    ]);
    ...
    console.error(
      "Error fetching data from API from football commentary route:",
      error.message
    );
    res.status(500).json({ error: "Unable to fetch football commentary data" });
  }
});
...
router.get("/statistics/:matchId", async (req, res) => {
  ...
  try {
    ...
    const [statsResponse] = await Promise.all([
      axios.get(`${FOOTAPI_URL}/api/match/${matchId}/statistics`, { headers }),
    ]);
    ...
    res.status(500).json({ error: "Unable to fetch football team stats data" });
  }
});
...
router.get("/h2h/:matchId", async (req, res) => {
  ...
  try {
    ...
    const [h2hResponse] = await Promise.all([
      axios.get(`${FOOTAPI_URL}/api/match/${matchId}/h2h`, { headers }),
    ]);
    ...
    res.status(500).json({ error: "Unable to fetch football team h2h data" });
  }
});
...
router.get("/table/:leagueId/:seasonId", async (req, res) => {
  ...
  try {
    ...
    const [tableResponse] = await Promise.all([
      axios.get(
        `${FOOTAPI_URL}/api/tournament/${leagueId}/season/${seasonId}/standings/total`,
        { headers }
      ),
    ]);
    ...
    if (tableData.length > 0) {
      for (const team of tableData[0].rows) {
        ...
        if (teamName && teamId) {
          const teamLogo = await fetchTeamLogo(teamId, teamName);
    ...
    res.status(500).json({ error: "Unable to fetch football table data" });
  }
});
...
router.get("/league/fixtures/:leagueId/:seasonId", async (req, res) => {
  ...
  try {
    ...
    const [fixturesResponse] = await Promise.all([
      axios.get(
        `${FOOTAPI_URL}/api/tournament/${leagueId}/season/${seasonId}/matches/next/${page}`,
        { headers }
      ),
    ]);
...
router.get("/league/results/:leagueId/:seasonId", async (req, res) => {
  ...
  try {
    ...
    const [resultsResponse] = await Promise.all([
      axios.get(
        `${FOOTAPI_URL}/api/tournament/${leagueId}/season/${seasonId}/matches/last/${page}`,
        { headers }
      ),
    ]);
...
router.get("/league/teamStats/:leagueId/:seasonId", async (req, res) => {
  ...
  try {
    ...
    const [resultsResponse] = await Promise.all([
      axios.get(
        `${FOOTAPI_URL}/api/tournament/${leagueId}/season/${seasonId}/best-teams`,
        { headers }
      ),
    ]);
...
router.get("/league/playerStats/:leagueId/:seasonId", async (req, res) => {
  ...
  try {
    ...
    const [resultsResponse] = await Promise.all([
      axios.get(
        `${FOOTAPI_URL}/api/tournament/${leagueId}/season/${seasonId}/best-players`,
        { headers }
      ),
    ]);
    ...
    console.error(
      "Error fetching data from API from league playerStats route:",
      error.message
    );
    res
      .status(500)
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [resultsResponse] = await Promise.all([
        axios.get(
          `${FOOTAPI_URL}/api/team/${teamId}/tournament/${leagueId}/season/${seasonId}/best-players`,
          { headers }
        ),
      ]);
      ...
      console.error(
        "Error fetching data from API from team playerStats route:",
        error.message
      );
      res
        .status(500)
...
router.get("/team/squad/:teamId", async (req, res) => {
  ...
  try {
    ...
    // Fetch data from API
    const [resultsResponse] = await Promise.all([
      axios.get(`${FOOTAPI_URL}/api/team/${teamId}/players`, { headers }),
    ]);
    ...
    resultsResponse?.data?.players?.map((player) => {
      ...
      if (player.player.position === "M") {
        midfielders.push(player.player);
      }
      if (player.player.position === "D") {
        defenders.push(player.player);
      }
      if (player.player.position === "G") {
        goalKeepers.push(player.player);
      }
    });
    const playersData = {
    ...
    res
      .status(500)
      .json({ error: "Unable to fetch football team squads data" });
  }
});
...
router.get("/team/fixtures/:teamId", async (req, res) => {
  ...
  try {
    ...
    // Fetch data from API
    const [fixturesResponse] = await Promise.all([
      axios.get(`${FOOTAPI_URL}/api/team/${teamId}/matches/next/${page}`, {
        headers,
      }),
    ]);
...
router.get("/team/results/:teamId", async (req, res) => {
  ...
  try {
    ...
    // Fetch data from API
    const [resultsResponse] = await Promise.all([
      axios.get(`${FOOTAPI_URL}/api/team/${teamId}/matches/previous/${page}`, {
        headers,
      }),
    ]);
...
router.get("/player/statistics/:matchId/:playerId", async (req, res) => {
  const { matchId, playerId } = req.params;
  console.log(matchId, playerId);

  try {
    const cachedMatchKey = `${matchId}/${playerId}stats`;
    ...
    const [statsResponse] = await Promise.all([
      axios.get(
        `${FOOTAPI_URL}/api/match/${matchId}/player/${playerId}/statistics`,
        { headers }
      ),
    ]);

    console.log("playerStats API call completed");
    const matchesData = {
      statsData: statsResponse?.data || {},
    };
...
router.get("/team/news/:teamName", async (req, res) => {
  // console.log(matchId);
  const { teamName } = req.params;

  try {
    const cachedMatchKey = `${teamName}footballNews`;
    ...
    console.error(
      "Error fetching data from API from football news route:",
      error.message
    );
    res.status(500).json({ error: "Unable to fetch football news data" });
  }
});
...
router.get("/news", async (req, res) => {
  ...
  try {
    const cachedMatchKey = `footballNews`;
...
// Add other football routes (details, commentary, statistics, etc.) similarly...
